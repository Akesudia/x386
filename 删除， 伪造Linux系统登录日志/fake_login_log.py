...
class FakeLog(object):
    def __init__(self, args):
        ...
        try:
            self.date = str(time.mktime(time.strptime(args.date, "%Y-%m-%d %H:%M:%S"))).split('.')[0] if args.date else None
        except ValueError, e:
            self.date = str(time.mktime(time.strptime(args.date + ':0', "%Y-%m-%d %H:%M:%S"))).split('.')[0]
            self.date_end = str(time.mktime(time.strptime(args.date + ":59", "%Y-%m-%d %H:%M:%S"))).split('.')[
            0] if args.date else None

        self.tty = args.tty
        self.pid = args.pid
        self.FILE_DICT = {'utmp': '/var/run/utmp', 'wtmp': '/var/log/wtmp', 'btmp': '/var/log/btmp', 'lastlog': '/var/log/lastlog'}
    ...
    def add_log(self):
        ...
        :return:
        """
        to_add_xtmp = [7, 13009, 'pts/4', 'ts/4', 'root', '10.1.100.10', 0, 0, 0, 1500475658, 498851, 23331082, 0, 0, 0]

        to_add_btmp = [6, 13732, 'ssh:notty', '', 'root', '10.1.100.1', 0, 0, 0, 1500311234, 0, 23331082, 0, 0, 0]
        ...
        if self.FILE_PATH.endswith('utmp') or self.FILE_PATH.endswith('wtmp'):
            ...
            if self.user:
                ...
                try:
                    ...
                    raise GeneralError('Cannot open file: {0}'.format(e.message))
...
if __name__ == '__main__':
    ...
    parse.add_argument('--mode', dest='mode', type=str, required=True, help='add, delete log')
    parse.add_argument('--type', dest='type', type=str, choices=['utmp', 'wtmp', 'btmp', 'lastlog'], required=True, help='utmp |wtmp |btmp |lastlog')
    ...
    parse.add_argument('--date', dest='date', type=str, help='login time 2017-7-20 15:30')
    parse.add_argument('--timestamp', dest='timestamp', type=str, help='login time 1500475126')
    parse.add_argument('--pid', dest='pid', type=str, default=random.randint(os.getpid() + 100, os.getpid() + 1000), help='process id, for add  only')
    ...
    if not any((argument.user, argument.host, argument.date, argument.timestamp)):
        raise GeneralError('you must choose at least user | host | date |timestamp as condition')
